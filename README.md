...
- Update your pricing plan data to align to your stripe data
  - See `/src/routes/(marketing)/fiyatlandırma/pricing_plans.ts` and Fill in all fields for each plan. stripe_price_id and stripe_product_id should only be omitted on a single “free” plan. Multiple free plans are not supported.
  ...
  - if you want an item highlighted on `/fiyatlandırma`, specify that plan ID in `/src/routes/(marketing)/fiyatlandırma/+page.svelte`
