<script lang="ts">
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch("/account/api/pdf", {
        method: "POST",
        body: formData,
      }).then((res) => res.json())
      ...
      while (!generatingComplete) {
        ...
        console.log(
          "RESPONSE FROM OPENAI === BEFORE CREATING CARDS",
        ...
        const cardResponse = await fetch("/account/api/card", {
          method: "POST",
          body: JSON.stringify({
            type: "createCard",
            data: {
  ...
  const createCardsYoutubeLink = async () => {
    ...
    try {
      ...
      const response = await fetch("/account/api/youtube", {
        method: "POST",
        body: JSON.stringify({
      ...
      for (let batchNumber = 1; batchNumber < totalBatches; batchNumber++) {
        ...
        console.log(
          "AI RESPONSE FLASHCARDS TO CREATE CARDS===",
